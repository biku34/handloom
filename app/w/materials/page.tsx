    ...
    <PortalShell title="Weaver" nav={WEAVER_NAV} userName={session?.name}>
        ...
        <div className="mt-5 grid grid-cols-3 divide-x divide-silk-200 rounded-2xl bg-white py-4 text-center ring-1 ring-silk-200">
          {[
            ...
            <div key={String(l)} className="px-2">
              ...
              <div className="mt-0.5 text-[10px] sm:text-[11px] font-semibold uppercase tracking-wide text-stone-500">{l}</div>
            </div>
          ))}
        </div>
      )}
      ...
      <div className="mt-5 grid gap-3 sm:gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {lots.map((l) => {
          ...
          return (
            <div key={l.lotId} className="rounded-2xl bg-white p-4 sm:p-5 ring-1 ring-silk-200">
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-start justify-between gap-2">
                    <p className="font-bold text-maroon-900">{TYPE_LABEL[l.type] || l.type}</p>
                    {l.spec?.isHankYarn && (
                      <span className="shrink-0 rounded-full bg-leaf-600/10 text-leaf-700 ring-1 ring-leaf-600/25 px-2 py-0.5 text-[10px] font-bold">HANK</span>
                    )}
                  </div>
                  <p className="mt-0.5 flex items-center gap-1.5 text-sm text-stone-600 truncate">
                    {l.spec?.colour && <span className="h-3 w-3 shrink-0 rounded-full ring-1 ring-black/10" style={{ background: swatch(l.spec.colour) }} />}
                    <span className="truncate">{[l.spec?.colour, l.supplier?.name && `from ${l.supplier.name}`].filter(Boolean).join(" · ") || "No colour or supplier"}</span>
                  </p>
                </div>
              </div>
              ...
              <div className="mt-4">
                ...
                <p className="mt-1 text-[11px] text-stone-500">{usedPct}% woven into your pieces{low ? " · running low" : ""}</p>
              </div>
              ...
              <div className="mt-3 flex flex-wrap items-center gap-1.5 border-t border-silk-100 pt-3">
                ...
                <span className="font-mono text-[11px] text-stone-400">{l.lotId}</span>
